...
import org.apache.spark.scheduler.{SparkListenerLogStart, SparkListenerEvent}
 ...
 */
class LoggingListener(sparkConf: SparkConf)
  extends SparkFirehoseListener with Logging {

  import LoggingListener._

  private[spark] def isEventLogEnabled: Boolean = sparkConf.getBoolean("spark.eventLog.enabled", false)

  private def appId = sparkConf.get("spark.app.id")
  private def appAttemptId = sparkConf.getOption("spark.yarn.app.attemptId")
  private def hadoopConf = SparkHadoopUtil.get.newConfiguration(sparkConf)
  private def logBaseDir = {
    val unresolvedDir =
      sparkConf.get("spark.eventLog.dir", LoggingListener.DEFAULT_LOG_DIR).stripSuffix("/")
    Utils.resolveURI(unresolvedDir)
  }
  ...
  private[spark] val loggedEvents = new ArrayBuffer[JValue]

  // Visible for tests only.
  private[spark] val logPath = getLogPath(logBaseDir, appId, appAttemptId, compressionCodecName)
  ...
  def start() {
    ...
    try {
      ...
      LoggingListener.initEventLog(bstream)
...
object LoggingListener extends Logging {
